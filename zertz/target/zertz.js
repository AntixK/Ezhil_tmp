...
var __name__ = 'zertz';
export var Zertz =  __class__ ('Zertz', [HexGrid], {
	__module__: __name__,
	get __init__ () {return __get__ (this, function (self, side_length, cell_type) {
		...
		__super__ (Zertz, '__init__') (self, side_length, cell_type);
	...
	get reset_canvas () {return __get__ (this, function (self) {
		...
		var posx = (CANVAS_WIDTH * 230) / 1000;
		posx += self.RING_RADIUS - 1;
		for (var i = 0; i < self.num_cells; i++) {
			var posy = (CANVAS_HEIGHT * 340) / 900 - ((__mod__ (self.cells_per_col [i], 4)) * self.RING_RADIUS) / 2;
			var ring = self.grid [i]._ring_params (__kwargtrans__ ({x: posx, y: posy, radius: self.RING_RADIUS}));
